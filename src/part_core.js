...
function sanitizeTeam(t) {
  ...
  const cards = t.cards.filter(c => c && String(c.name || '').trim()).map(c => ({
    ...
    photo: String(c.photo || ''), sq: String(c.sq || ''), pos: String(c.pos || ''), home: String(c.home || ''),
    major: String(c.major || ''), career: String(c.career || '') }));
  for (const c of cards) if (c.sec === 'player' && c.badge) c.sec = 'captain';
  return cards.length ? { v: 1, ts: Number(t.ts) || 0, cards } : null;
}
function activeTeam() { return teamLocal || repoTeam || DEFAULT_TEAM; }
/* PUBLISHED WINS. A device's working copy only survives if it holds edits
   made AFTER the published file was generated (its ts stamp) — otherwise a
   publish is a fresh start for every device, no buttons to press. */
function reconcileTeam() {
  if (repoTeam && teamLocal && !(Number(teamLocal.ts || 0) > Number(repoTeam.ts || 0))) {
...
function overallOf(sp, sk, keeper) {
  const cap = (v, per, max) => Math.max(0, Math.min(max, max * (v / per)));
  if (keeper && sk && sk.faced && sk.gp) {
...
function bbCard(c, idx) {
  ...
  const entries = [['Squadron', c.sq], ['Position', c.pos], ['Hometown', c.home], ['Major', c.major], ['Intended Career Field', c.career]]
  ...
  if (sp || sk || (idx && idx.onRoster.has(k))) {
    ...
    if (keeperCard && sk && sk.faced) {          // a keeper reads as a keeper
...
function saveTeamLocal() { teamLocal.ts = Date.now(); lsSet('afahb.team.v1', teamLocal); renderTeamCards(); }
function renderTmList() {
  ...
  if (teamLocal) host.appendChild(el('div', { cls: 'small', style: 'color:#8a6502;font-weight:600;padding:8px 4px 0',
    text: 'This device holds unpublished edits — DOWNLOAD team.json and upload it to data/ to publish, or DISCARD to sync with the site.' }));
}
...
$('#tm-save').addEventListener('click', () => {
  ...
  const card = {
    ...
    photo: tmPhoto || '', sq: $('#tm-sq').value.trim(), pos: $('#tm-pos').value.trim(), home: $('#tm-home').value.trim(),
...
$('#tm-download').addEventListener('click', () => {
  // the stamp makes this file supersede every device's older working copy
  const data = JSON.stringify(Object.assign({}, activeTeam(), { ts: Date.now() }), null, 1);
...
function sanitizeGallery(t) {
  ...
  return photos.length ? { v: 1, ts: Number(t.ts) || 0, photos } : null;
}
...
function reconcileGallery() {          // published wins — same rule as the cards
  if (repoGallery && galLocal && !(Number(galLocal.ts || 0) > Number(repoGallery.ts || 0))) {
...
function saveGalLocal() { galLocal.ts = Date.now(); lsSet('afahb.gallery.v1', galLocal); renderGallery(); }
...
$('#gal-download').addEventListener('click', () => {
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([JSON.stringify(Object.assign({}, activeGallery(), { ts: Date.now() }), null, 1)], { type: 'application/json' }));
